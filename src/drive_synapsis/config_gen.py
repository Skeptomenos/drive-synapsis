...
# Try imports
try:
    from drive_synapsis.auth import get_creds
    from drive_synapsis.auth.oauth_config import get_credentials_dir
...
def confirm(prompt: str, default: bool = True) -> bool:
    ...
    response = (
        input(f"{Colors.BOLD}{prompt}{Colors.ENDC} [{default_str}]: ").strip().lower()
    )
    if not response:
        return default
...
def setup_credentials():
    ...
    if client_secret.exists():
        ...
        while True:
            path_str = get_input(
                "Path to your downloaded client_secret.json (or 'skip')"
            )
            if path_str.lower() == "skip":
                print_warning(
                    "Skipping credential setup. Server will not work without it."
                )
                return
...
def configure_clients(uv_path: str, project_root: str):
    ...
    print(
        "3. OpenCode (Global ~/.config/opencode/opencode.json OR Local ./opencode.json)"
    )
...
def _setup_gemini(uv_path: str, project_root: str):
    ...
    if isinstance(config.get("mcpServers"), list):
        print_warning(
            "Your Gemini config uses a list for 'mcpServers'. Converting to object not supported automatically to avoid breaking."
        )
...
def main():
    ...
    if uv_path == "uv":
        print_warning(
            "Could not locate absolute path for 'uv'. Using 'uv' command directly."
        )
